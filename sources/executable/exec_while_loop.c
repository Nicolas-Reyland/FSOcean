...
int exec_while_loop(struct ExecWhileLoop while_loop) {
    int result = 0;
    while ((result = exec_executables(while_loop.condition_commands, while_loop.num_condition_commands) & EXEC_FLAG_EXIT_CODE_MASK) == 0) {
        result = exec_executables(while_loop.body_commands, while_loop.num_body_commands);
